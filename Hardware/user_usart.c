...
static inline void usart_clock_init(USART_TypeDef *usart){
    ...
    else if(usart == USART6) RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART6, ENABLE);
...
static inline uint8_t Get_irq(USART_TypeDef *usart){
    ...
    else if (usart == USART2) return USART2_IRQn;
    else if (usart == USART3) return USART3_IRQn;
    else if (usart == UART4) return UART4_IRQn;
    else if (usart == UART5) return UART5_IRQn;
    else if (usart == USART6) return USART6_IRQn;
    #if defined(STM32F427_437xx)
    else if (usart == UART7) return UART7_IRQn;
    else if (usart == UART8) return UART8_IRQn;
    #endif
    else  return USART1_IRQn;

}
Usart_Device usart_create(int baud,USART_TypeDef *usart,GPIO_TypeDef *rx_port,GPIO_TypeDef *tx_port,uint16_t rx_pin,uint16_t tx_pin){
    if(baud<0){
        baud=-baud;
    }
    Usart_Device device={
...
void user_usart_init(Usart_Device *device){
   ...
   usart_clock_init(device->usart);
    ...
    GPIO_PinAFConfig(device->RX_port,Get_Source_Pin(device->RX_pin),Get_AF_val(device->usart));
    GPIO_PinAFConfig(device->TX_port,Get_Source_Pin(device->TX_pin),Get_AF_val(device->usart));
...
// }
#if (RECV_NVIC)
void usart_nvic_register(Usart_Device *device,void(*func)(uint8_t data,void *context),void *context){
